...
class ImageUploader(private val context: Context) {
    ...
    suspend fun uploadFromGallery(uri: Uri): Result<ImageModel> = withContext(Dispatchers.IO) {
        Log.d("ImageUploader", "Starting upload for URI: $uri")
        try {
            if (uri.scheme.isNullOrEmpty()) {
                Log.e("ImageUploader", "Invalid URI scheme for URI: $uri")
            ...
            Log.d("ImageUploader", "Upload successful for URI: $uri")
            Result.success(imageModel)
        } catch (e: Exception) {
            Log.e("ImageUploader", "Upload failed for URI: $uri", e)
            // Retry once
            try {
                delay(1000)
                val imageModel = ImageModel(
                ...
                Log.d("ImageUploader", "Upload successful after retry for URI: $uri")
                Result.success(imageModel)
            } catch (e2: Exception) {
                Log.e("ImageUploader", "Upload failed after retry for URI: $uri", e2)
...
suspend fun uploadFromCamera(file: File): Result<ImageModel> = withContext(Dispatchers.IO) {
    Log.d("ImageUploader", "Starting upload for file: ${file.absolutePath}")
    try {
        if (!file.exists()) {
            Log.e("ImageUploader", "Camera file does not exist: ${file.absolutePath}")
            return@withContext Result.failure(Exception("Camera file does not exist"))
        }
        if (file.length() > 10 * 1024 * 1024) {
            Log.e("ImageUploader", "File size exceeds 10MB: ${file.absolutePath}")
        ...
        Log.d("ImageUploader", "Upload successful for file: ${file.absolutePath}")
        Result.success(imageModel)
    } catch (e: Exception) {
        Log.e("ImageUploader", "Upload failed for file: ${file.absolutePath}", e)
