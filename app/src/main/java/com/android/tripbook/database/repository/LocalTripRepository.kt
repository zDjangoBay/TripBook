...
/**
 * Repository for Trip data using Room database
 * Provides clean API for accessing trip data locally
 */
class LocalTripRepository(private val tripDao: TripDao) {
    
    /**
     * Get all trips as Flow for reactive UI updates
     */
    ...
    suspend fun getAllTripsOnce(): List<Trip> {
        return tripDao.getAllTripsOnce().map { it.toTrip() }
    }
     ...
     */
    fun getTripByIdFlow(tripId: Int): Flow<Trip?> {
        return tripDao.getTripByIdFlow(tripId).map { entity ->
            entity?.toTrip()
        }
    }
    ...
    suspend fun insertTrip(trip: Trip) {
        tripDao.insertTrip(TripEntity.fromTrip(trip))
    }
    ...
    suspend fun insertTrips(trips: List<Trip>) {
        val entities = trips.map { TripEntity.fromTrip(it) }
    ...
    suspend fun updateTrip(trip: Trip) {
        tripDao.updateTrip(TripEntity.fromTrip(trip))
    }
    ...
    suspend fun deleteTrip(trip: Trip) {
        tripDao.deleteTrip(TripEntity.fromTrip(trip))
    }
    ...
    fun searchTrips(query: String): Flow<List<Trip>> {
        return tripDao.searchTrips(query).map { entities ->
            entities.map { it.toTrip() }
        }
    }
    ...
    suspend fun seedDataIfEmpty() {
        if (getTripsCount() == 0) {
            val sampleTrips = getSampleTrips()
            insertTrips(sampleTrips)
        }
    }
    
    /**
     * Get sample trips data (same as existing SampleTrips)
     */
